...
from geopy.distance import geodesic as GD  # библиотека для вычисления расстояния по широте и долготе
...
def get_id_city_bestdeal(city):  # функция получения айди введенного города
    """
...
def get_hotels_bestdeal(id_city, num_hotels, min_price, max_price, max_distance, check_in, check_out,
                        coordinates):  # функция получения отелей в городе, по полученному айди
    """
    ...
    :param max_distance: максимальное расстояние отеля от центра
    :param coordinates: координаты центра выбранного района
    ...
    querystring = {"destinationId": id_city, "pageNumber": "1", "pageSize": num_hotels,
                   ...
                   "priceMin": min_price, "priceMax": max_price, "sortOrder": "PRICE", "locale": "ru_RU",
                   "currency": "RUB"}

    response = requests.request("GET", url, headers=headers, params=querystring)
    data_site_hotels = response.text  # полученные данные с сайта
    converted_data = json.loads(data_site_hotels)  # конвертированные данные для удобной работы
    ...
    for hotel in data_hotel:
        coordinates_hotel = hotel['coordinate']['lat'], hotel['coordinate']['lon']  # получаем координаты отеля
        distance_hotel_before_center = GD(coordinates,
                                          coordinates_hotel).km  # вычисляем расстояние от отеля до центра выбранного района
        if distance_hotel_before_center <= max_distance:
            ...
            distance = math.ceil(distance_hotel_before_center)

            hotels_list[hotel_id] = [
                '{name},\nАдрес: {city}, {address}\nСтоимость за ночь: {price_in_night} RUB\nСтоимость: {price}, {info}\nРасстояние до центра : {distance} км'.format(
                    name=hotel_name, city=city, address=address, price_in_night=price_in_night, price=price,
